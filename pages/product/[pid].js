...
export async function getServerSideProps({ query }) {
  ...
  const pid = query.pid;
  const res = await fetch(`${server}/api/Property/getProductDetail/${pid}`);
        ...
        <div className="container">
          ...
          <div className="product-single__info">
            <div className="product-single__info-btns">
              <button type="button" onClick={() => setShowBlock('description')} className={`btn btn--rounded ${showBlock === 'description' ? 'btn--active' : ''}`}>รายละเอียด</button>
